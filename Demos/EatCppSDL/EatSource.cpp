...
int main();                             // Declarations for functions to be used later.
bool initSDL();
void mainloop();
void exitvoid();
void render();
...
int gamespeed;                          // Used later in the timing of the main loop. Defaults at 50, change up with "," down with "."
...
int main(int argc, char *argv[])
{
    ...
    srand(static_cast<unsigned>(time(
                                    0))); // Seed the random number generator with the time so that the numbers will be different every time the game is run.
    if (initSDL() ==
...
bool initSDL()
{
    ...
    {
        printf("Unable to initialize SDL: ", SDL_GetError(), endl);
        return true;
    }
