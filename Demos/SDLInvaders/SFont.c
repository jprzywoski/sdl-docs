...
Uint32 GetPixel(SDL_Surface *Surface, Sint32 X, Sint32 Y)
{
    ...
    if (X < 0)
    {
        puts("SFONT ERROR: x too small in GetPixel. Report this to <karlb@gmx.net>");
    }
    if (X >= Surface->w)
    {
        puts("SFONT ERROR: x too big in GetPixel. Report this to <karlb@gmx.net>");
    }
...
void InitFont2(SFont_FontInfo *Font)
{
    ...
    while (x < Font->Surface->w)
    {
        ...
        {
            Font->CharPos[i++] = x;
            while ((x < Font->Surface->w - 1) && (GetPixel(Font->Surface, x, 0) == SDL_MapRGB(Font->Surface->format,
                                                  255, 0, 255)))
            {
                x++;
            }
...
void PutString2(SDL_Surface *Surface, SFont_FontInfo *Font, int x, int y, char *text)
{
    ...
    while (text[i] != '\0')
    {
        ...
        else
        {
            ...
            /*	    printf("printing %c %d\n",text[i],ofs);  */
            srcrect.w = dstrect.w = (Font->CharPos[ofs + 2] + Font->CharPos[ofs + 1]) / 2 - (Font->CharPos[ofs] +
...
void SFont_InternalInput(SDL_Surface *Dest, SFont_FontInfo *Font, int x, int y, int PixelWidth, char *text)
{
    ...
    Back = SDL_AllocSurface(Dest->flags, Dest->w, Font->h, Dest->format->BitsPerPixel, Dest->format->Rmask,
