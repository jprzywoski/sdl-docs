...
int InitializeVideo(void) {
    ...
    p->ScreenSurface = SDL_SetVideoMode(640, 480, 32, SDL_SWSURFACE | SDL_RESIZABLE     /*	| SDL_FULLSCREEN	*/
	);
    ...
    p->ViewportOffset.x = p->ViewportOffset.y = 0;

    /*	Make the planet surfaces pool	*/
    g.PlanetPool = BlitPool_CreatePool(NULL);

    BlitPool_PostByDescription(g.PlanetPool, p->PlanetSurface, "pos=(0,0):" "T:(29,0)-(170,200):" "T:(0,29)-(200,170):"
    "O:(29,29)-(170,170):");
...
int EventProcess(void) {
	...
	case SDL_VIDEORESIZE:
	    g.Video.ScreenSurface = SDL_SetVideoMode(event.resize.w, event.resize.h, surf->format->BitsPerPixel,
		surf->flags);
...
void Clear(void) {
    ...
    g.MouseState.Acceleration.x = g.MouseState.Acceleration.y = 0;

    g.Video.ViewportOffset.x = g.Video.ViewportOffset.y = 0;
}
...
void UpdateTitleBar(void) {
    ...
    sprintf(str, "%s | %2lu fps | [%lu x %lu x %lu bit] | %lu planet | [space] reset | [s] stop | [b]UsePool=%d", title,
    g.Video.FramePerSecond, surf->clip_rect.w, surf->h, surf->format->BitsPerPixel, g.PlanetCount,
    g.Option.UseBlitPool);
...
void RenderUseBlitPool(void) {
    ...
    InitAllocator();
    for (i = 0; i < g.PlanetCount; i++) {
	BlitPool_PostFill(pool, &g.PlanetArray[i].ClearRect, SDL_MapRGBA(screen->format, 0, 0, 0, 255),
	BLIT_ALPHA_OPAQUE);
    }
    for (i = 0; i < g.PlanetCount; i++) {
	rect.x = g.PlanetArray[i].Position.x - offset - g.Video.ViewportOffset.x;
