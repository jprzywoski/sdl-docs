...
#endif
enum { MUSIC_WAV, SHOT_WAV, EXPLODE_WAV, NUM_WAVES };
...
{
    if ((sprite1->y >= (sprite2->y + sprite2->image->h)) || (sprite1->x >= (sprite2->x + sprite2->image->w)) ||
            (sprite2->y >= (sprite1->y + sprite1->image->h)) || (sprite2->x >= (sprite1->x + sprite1->image->w)))
    {
        return 0;
    }
    return 1;
    ...
    while (player.alive)
    {
        ...
        for (i = 0; i < MAX_ALIENS; ++i)
        {
            if (aliens[i].alive)
            {
                ...
                else if (aliens[i].x >= (screen->w - aliens[i].image->w))
                {
                    aliens[i].x = (screen->w - aliens[i].image->w) - 1;
                    aliens[i].y += aliens[i].image->h;
                    aliens[i].facing = -1;
        ...
        for (j = 0; j < MAX_SHOTS; ++j)
        {
            for (i = 0; i < MAX_ALIENS; ++i)
            {
                if (shots[j].alive && aliens[i].alive && Collide(&shots[j], &aliens[i]))
                {
                    aliens[i].alive = 0;
