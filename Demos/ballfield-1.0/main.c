...
SDL_Surface *clean_alpha(SDL_Surface *s)
{
    ...
    work = SDL_CreateRGBSurface(SDL_SWSURFACE, s->w, s->h, 32, 0xff000000, 0x00ff0000, 0x0000ff00, 0x000000ff);
    if (!work)
    {
        return NULL;
    }
...
void ballfield_render(ballfield_t *bf, SDL_Surface *screen)
{
    ...
    for (i = 0; i < BALLS; ++i)
    {
        ...
        SDL_BlitSurface(bf->gfx[bf->points[j].c], &bf->frames[f], screen, &r);
        if (--j < 0)
        {
            j = BALLS - 1;
        }
...
int main(int argc, char *argv[])
{
    ...
    SDL_Event event;
    int bpp = 0, flags = SDL_DOUBLEBUF | SDL_SWSURFACE, alpha = 1;
    int x_offs = 0, y_offs = 0;
    long tick, last_tick, last_avg_tick;
    double t = 0;
    float dt;
    int i;
    float fps = 0.0;
    ...
    if (ballfield_load_gfx(balls, "blueball.png", 0) || ballfield_load_gfx(balls, "redball.png", 1))
    {
    ...
    SDL_SetColorKey(temp_image, SDL_SRCCOLORKEY | SDL_RLEACCEL, SDL_MapRGB(temp_image->format, 255, 0, 255));
