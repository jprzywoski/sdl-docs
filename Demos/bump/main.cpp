...
// ----------------------------------------------------------------------//
int loadpcx(const char *nomfich, unsigned long taille, int *image)
{
    ...
    do
    {
        ...
        if ((data & 0xC0) == 0xC0)
        {
            nb_octets = (data & 0x3F);
            fread(&data, 1, 1, fichpcx);
            for (indexrle = 1; indexrle <= nb_octets; indexrle++)
            {
                image[index++] = (data << 16) | (data << 8) | data;
            }
        }
        else
        {
...
int main(int argc, char *argv[])
{
    ...
    // Cette table na pas besoin d'�tre optimis�e.
    for (y = 0; y < 256; y++)
    {
        for (x = 0; x < 256; x++)
        {
            ...
            // changer de rep�re.
            nEnvMap[x + 256 * y] = 255 - 255 * sqrt((x - 128) * (x - 128) + (y - 128) * (y - 128)) / sqrt(128 * 128 +
                                   128 * 128);
        }
    }
    // G�n�ration d'une palette qui va nous donner la couleur
    // a utiliser en fonction d'un niveau d'ullimination donn�.
    for (i = 0; i < 64; i++)
    {
        pal[i] = 0;
    }
    ...
    // Ouverture de l'image qui va �tre bump�e
    loadpcx("bump.pcx", 320 * 200, image);

    lX = 160 + 80 * cos(fTime += .1);
    lY = 100 + 80 * sin(fTime);
    // G�n�ration du bump mapping.
    do
    {
        ...
        for (y = 1; y < 199; y++)
        {
            for (x = 1; x < 319; x++)
            {
                ...
                // On r�cup�re le niveau d'illumination correspondant.
                // puis la couleur qui lui est associ�e.

                // On effectue un double changement de rep�re.
                dX = dX - (lX - x);
                dY = dY - (lY - y);
                // On clip les coordon�es.
                if (dX <= -128 || dX >= 128)
                {
                    dX = dY = -128;
                }
                ...
                // On retourne dans un rep�re classique.
                dX += 128;
                dY += 128;
