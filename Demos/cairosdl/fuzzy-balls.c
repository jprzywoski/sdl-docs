...
static void init_bobs(struct bob *bobs, size_t num_bobs)
{
    size_t i;
...
static void alloc_bobs(struct bob *bobs, size_t num_bobs)
{
    size_t i;
    ...
    for (i = 0; i < num_bobs; i++)
    {
        ...
        {
            bob->surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, width, height);
        }
        else
        {
            SDL_Surface *sdl_surface = SDL_CreateRGBSurface(SDL_SWSURFACE | SDL_SRCALPHA, width, height, 32,
...
static void render_bob(struct bob *bob, int i)
{
    ...
    cairo_set_source_rgba(cr, 1 - fabs(dx), 1 - fabs(dy), theta, (1 - 2 * fabs(dx)) * (1 - 2 * fabs(dy)));
    if (1)
    {
        cairo_pattern_t *pat;

        pat = cairo_pattern_create_radial(-dx, -dy, 0.0, 0, 0, 1.0);
        cairo_pattern_add_color_stop_rgba(pat, 0.0, fabs(dy), 0, fabs(dx), 0);

        cairo_pattern_add_color_stop_rgba(pat, bob->focus, fabs(dx + dy), i & 1 ? 1 : 1, i & 1 ? fabs(dx) : 1 - fabs(
                                              dx), 0.6);

        cairo_pattern_add_color_stop_rgba(pat, 1.0, 1 - fabs(dy), 1 - fabs(dx), theta, 0.0);
...
static void render_bobs(struct bob *bobs, size_t num_bobs)
{
    size_t i;
...
static void blit_bobs_using_sdl(struct bob *bobs, size_t num_bobs)
{
    size_t i;
    SDL_Surface *screen = SDL_GetVideoSurface();
...
static void blit_bobs_using_cairo(struct bob *bobs, size_t num_bobs)
{
    size_t i;
    SDL_Surface *screen = SDL_GetVideoSurface();
    cairo_t *cr;
...
static double sim_bobs(struct bob *bobs, size_t num_bobs, double t0, double t1)
{
    double dt = 0.002;
    double G = 0.5;
    double t = t0;
...
static void on_expose(struct bob *bobs, size_t num_bobs)
{
    SDL_Surface *screen = SDL_GetVideoSurface();

    SDL_FillRect(screen, NULL, SDL_MapRGBA(screen->format, 0, 0, 0, SDL_ALPHA_OPAQUE));
    if (BLIT_BOBS_USING_CAIRO)
    {
