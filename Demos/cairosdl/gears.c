...
static void gear(cairo_t *cr, double inner_radius, double outer_radius, int teeth, double tooth_depth)
{
    ...
    for (i = 1; i <= teeth; i++)
    {
        ...
        if (i < teeth)
        {
            cairo_line_to(cr, r1 * cos(angle + 3 * da), r1 * sin(angle + 3 * da));
        }
    }
    cairo_close_path(cr);
...
void trap_setup(cairo_surface_t *target, int w, int h)
{
    ...
    {
        animpts[i + 0] = (float)(DRAND * w);
        animpts[i + 1] = (float)(DRAND * h);
        deltas[i + 0] = (float)(DRAND * 6.0 + 4.0);
        deltas[i + 1] = (float)(DRAND * 6.0 + 4.0);
...
static void stroke_and_fill_animate(double *pts, double *deltas, int index, int limit)
{
    ...
    {
        newpt = -newpt;
        deltas[index] = (double)(DRAND * 4.0 + 2.0);
    }
...
static void stroke_and_fill_step(int w, int h)
{
    int i;
...
static Uint32 print_fps_timer(Uint32 interval, void *param)
{
