...
static int sdl_surface_eq(SDL_Surface *a, SDL_Surface *b)
{
    if (a->format->BitsPerPixel != 32)
    {
        return 0;
    }
    if (a->format->BytesPerPixel != 4)
    {
        return 0;
    }
    if (SDL_MUSTLOCK(a))
...
#define check_field(name) if (a->name != b->name) \
	return 0
...
static SDL_Surface *dup_sdl_surface(SDL_Surface *a)
{
    SDL_Surface *b = SDL_CreateRGBSurface(SDL_SWSURFACE, a->w, a->h, a->format->BitsPerPixel, a->format->Rmask,
                                          a->format->Gmask, a->format->Bmask, a->format->Amask);
    unsigned char *b_bytes = (unsigned char *)(b->pixels);
    unsigned char *a_bytes = (unsigned char *)(a->pixels);
    size_t row_size = a->format->BytesPerPixel * a->w;
    int i;
...
static int test_argb32()
{
    SDL_Surface *ref;
    SDL_Surface *sdlsurf = SDL_CreateRGBSurface(SDL_SWSURFACE, 100, 100, 32, CAIROSDL_RMASK, CAIROSDL_GMASK,
                           CAIROSDL_BMASK, CAIROSDL_AMASK);

    /* Prefilling the surface tests the mark_dirty functions. */
    SDL_FillRect(sdlsurf, NULL, SDL_MapRGBA(sdlsurf->format, 255, 0, 0, 128));
    ...
    /* Make the reference image. */
    {
        ...
        r.x = r.y = 25;
        r.w = r.h = 50;
        SDL_FillRect(ref, &r, SDL_MapRGBA(ref->format, 255, 170, 0, 192));
    }
