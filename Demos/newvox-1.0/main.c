...
void ComputeMap(void)
{
    ...
    for (k = 0; k < 3; k++)
        for (i = 0; i < 256 * 256; i += 256)
            for (j = 0; j < 256; j++)
            {
                HMap[i + j] = (HMap[((i + 256) & 0xFF00) + j] + HMap[i + ((j + 1) & 0xFF)] + HMap[((i - 256) & 0xFF00) +
                               j] + HMap[i + ((j - 1) & 0xFF)]) >> 2;
            }
...
void View(int x0, int y0, float aa, SDL_Surface *screen)
{
    ...
    {
        Line(x0 + d * 65536 * cos(aa - FOV), y0 + d * 65536 * sin(aa - FOV), x0 + d * 65536 * cos(aa + FOV), y0 + d *
             65536 * sin(aa + FOV), h - 30, 100 * 256 / (d + 1));
    }
...
main(int argc, char *argv[])
{
    ...
    screen = SDL_SetVideoMode(320, 200, 8, (SDL_HWSURFACE | SDL_HWPALETTE));
    if (screen == NULL)
    {
