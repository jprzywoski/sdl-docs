...
int main(int argc, char *argv[])
{
    ...
    int total_calls, total_blits, total_recursions, total_pixels;
    int peak_calls = 0, peak_blits = 0, peak_recursions = 0, peak_pixels = 0;
    int i;
    long tick1, tick2;
    float dt;
    ...
    while (SDL_PollEvent(&event) >= 0)
    {
        ...
        if (verbose >= 3)
        {
            for (i = 0; i < num_of_layers; ++i)
            {
                printf("%4d %8d %8d %8d %8d\n", i, layers[i].calls, layers[i].blits, layers[i].recursions,
                       layers[i].pixels);
            }
        }
        for (i = 0; i < num_of_layers; ++i)
        ...
        if (verbose >= 2)
        {
            printf("TOTAL:%7d %8d %8d %8d\n", total_calls, total_blits, total_recursions, total_pixels);
        }
        if (verbose >= 1)
        {
            printf("PEAK:%8d %8d %8d %8d\n", peak_calls, peak_blits, peak_recursions, peak_pixels);
        }
