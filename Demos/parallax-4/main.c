...
{
    /*	 0123456789ABCDEF */
    "3333333333333333", "3   2   3      3", "3   222 3  222 3", "3333 22     22 3", "3       222    3",
    "3   222 2 2  333", "3   2 2 222    3", "3   222      223", "3        333   3", "3  22 23 323  23",
    "3  22 32 333  23", "3            333", "3 3  22 33     3", "3    222  2  3 3", "3  3     3   3 3",
    "3333333333333333"
};
map_data_t single_map =
{
    /*	 0123456789ABCDEF */
    "3333333333333333", "3000200030000003", "3000222030022203", "3333022000002203", "3000000022200003",
    "3000222020200333", "3000202022200003", "3000222000000223", "3000000003330003", "3002202303230023",
    "3002203203330023", "3000000000000333", "3030022033000003", "3000022200200303", "3003000003000303",
    "3333333333333333"
};
...
int main(int argc, char *argv[])
{
    ...
    layer_t *layers;
    SDL_Event event;
    int bpp = 0, flags = 0, verbose = 0, use_planets = 1, num_of_layers = 7, bounce_around = 0, wrap = 0, alpha = 0;
    int total_blits, total_recursions, total_pixels;
    int peak_blits = 0, peak_recursions = 0, peak_pixels = 0;
    int i;
    long tick1, tick2;
    float dt;
    ...
    SDL_WM_SetCaption("Parallax Scrolling 4 - Overdraw +", "Parallax 4");
    SDL_ShowCursor(0);
    ...
    if (num_of_layers > 1)
    {
        ...
        for (i = 1; i < num_of_layers - 1; ++i)
            if ((i & 1) && use_planets)
            {
                layer_init(&layers[i], &middle_map, tiles, otiles);
            }
            else
            {
                layer_init(&layers[i], &foreground_map, tiles, otiles);
            }
        layer_init(&layers[num_of_layers - 1], &background_map, tiles, otiles);
        /*
    ...
    while (SDL_PollEvent(&event) >= 0)
    {
        ...
        if (num_of_layers > 1)
        {
            layer_vel(&layers[num_of_layers - 1], sin(tick1 * 0.00011f) * BACKGROUND_VEL, cos(tick1 * 0.00013f) *
                      BACKGROUND_VEL);
        }
        /* Animate all layers */
        ...
        if (verbose >= 3)
        {
            for (i = 0; i < num_of_layers; ++i)
            {
                printf("%4d %8d %8d %8d\n", i, layers[i].blits, layers[i].recursions, layers[i].pixels);
            }
        }
        for (i = 0; i < num_of_layers; ++i)
        {
        ...
        if (verbose >= 2)
        {
            printf("TOTAL:%8d %8d %8d\n", total_blits, total_recursions, total_pixels);
        }
        if (verbose >= 1)
        {
            printf("PEAK:%8d %8d %8d\n", peak_blits, peak_recursions, peak_pixels);
        }
...
void layer_init(layer_t *lr, map_data_t *map, SDL_Surface *tiles, SDL_Surface *opaque_tiles)
{
    lr->next = NULL;
            ...
            pos.y < max_y; pos.y += TILE_H)
    {
                ...
                pos.x < max_x;)
        {
            ...
            if (detect_runs)
            {
                ...
                {
                    int tt = (*lr->map)[my][(mx + run_w) % MAP_W];
            ...
            while (run_w--)
            {
                ...
                if (tm != TM_EMPTY)
                {
                    ...
                    ++lr->blits;
                    lr->pixels += draw_tile(screen, tiles, pos.x, pos.y, tile);
                }
                ++mx;
                pos.x += TILE_W;
            }
