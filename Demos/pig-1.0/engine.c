...
PIG_engine *pig_open(SDL_Surface *screen)
{
    ...
    {
        pe->buffer = SDL_CreateRGBSurface(SDL_SWSURFACE, screen->w, screen->h, screen->format->BitsPerPixel,
    ...
    pe->sprites = (PIG_sprite **)calloc(PIG_MAX_SPRITES, sizeof (PIG_sprite *));
    if (!pe->sprites)
    {
        pig_close(pe);
        return NULL;
...
int pig_sprites(PIG_engine *pe, const char *filename, int sw, int sh)
{
    ...
    for (y = 0; y <= tmp->h - sh; y += sh)
        for (x = 0; x <= tmp->w - sw; x += sw)
        {
            ...
            s->hoty = sh / 2;
            s->radius = (sw + sh) / 5;
            tmp2 = SDL_CreateRGBSurface(SDL_SWSURFACE, sw, sh, 32, 0xff000000, 0x00ff0000, 0x0000ff00, 0x000000ff);
            ...
            SDL_SetAlpha(tmp2, SDL_SRCALPHA | SDL_RLEACCEL, SDL_ALPHA_OPAQUE);
            s->surface = SDL_DisplayFormatAlpha(tmp2);
            if (!s->surface)
            {
                fprintf(stderr, "Could not convert sprite %d" " of '%s'!\n", count, filename);
                return -1;
            }
...
static void test_offscreen(PIG_engine *pe, PIG_object *po, PIG_sprite *s)
{
    ...
    if (ev.cinfo.sides)
    {
        ...
        else if (ev.cinfo.sides & PIG_BOTTOM)
        {
            ...
            if (dy)
            {
                ev.cinfo.x = po->ip.ox + dx * (ev.cinfo.y - po->ip.oy) / dy;
            }
        }
        ...
        else if (ev.cinfo.sides & PIG_RIGHT)
        {
            ...
            if (dx)
            {
                ev.cinfo.y = po->ip.oy + dy * (ev.cinfo.x - po->ip.ox) / dx;
            }
        }
...
static void test_sprite_sprite(PIG_engine *pe, PIG_object *po, PIG_sprite *s)
{
    ...
    for (po2 = po->next; po2; po2 = next2)
    {
        ...
        if (!(po->hitmask & po2->hitgroup) && !(po2->hitmask & po->hitgroup))
        {
            continue;
        }
 ...
 * but incorrect.)
 */
int pig_test_map_vector(PIG_engine *pe, int x1, int y1, int x2, int y2, int mask, PIG_cinfo *ci)
{
    PIG_cinfo lci;
    PIG_map *m = pe->map;
    int x, y;
    ...
    if (ci->sides)
    {
        ci->ff = sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1) / dist);
    }
    return ci->sides;
}
...
static void test_sprite_map(PIG_engine *pe, PIG_object *po, PIG_sprite *s)
{
    ...
    if (pig_test_map_vector(pe, po->ip.ox, po->ip.oy, po->x, po->y, po->tilemask, &ev.cinfo))
    {
...
static void tile_area(PIG_engine *pe, SDL_Rect *r)
{
    ...
    for (y = starty; y <= maxy; ++y)
        for (x = startx; x <= maxx; ++x)
        {
            ...
            SDL_BlitSurface(pe->map->tiles, &from, pe->surface, &to);
        }
}
...
void draw_sprites(PIG_engine *pe)
{
    ...
    while (po)
    {
        ...
        if ((po->ip.gimage >= 0) && (po->ip.gimage < pe->nsprites))
        {
            ...
            SDL_BlitSurface(pe->sprites[po->ip.gimage]->surface, NULL, pe->surface, &dr);
            /*
...
static void show_rects(PIG_engine *pe, PIG_dirtytable *pdt)
{
    ...
    if (!pe->buffer)
    {
        pe->buffer = SDL_CreateRGBSurface(SDL_SWSURFACE, pe->screen->w, pe->screen->h, pe->screen->format->BitsPerPixel,
                                          pe->screen->format->Rmask, pe->screen->format->Gmask, pe->screen->format->Bmask, pe->screen->format->Amask);
        if (!pe->buffer)
        {
            return;
        }
...
void pig_flip(PIG_engine *pe)
{
    ...
    else if (pe->surface == pe->buffer)
    {
        for (i = 0; i < pdt->count; ++i)
        {
            SDL_BlitSurface(pe->buffer, pdt->rects + i, pe->screen, pdt->rects + i);
        }
    }
    if ((pe->screen->flags & SDL_HWSURFACE) == SDL_HWSURFACE)
    {
...
void pig_draw_sprite(PIG_engine *pe, int frame, int x, int y)
{
    ...
    SDL_BlitSurface(pe->sprites[frame]->surface, NULL, pe->surface, &dr);
}
...
int pig_map_from_string(PIG_map *pm, const char *trans, const char *data)
{
    ...
    for (y = 0; y < pm->h; ++y)
        for (x = 0; x < pm->w; ++x)
        {
            ...
            if (!f)
            {
                fprintf(stderr, "Character '%c' not in" " the translation string!\n", c);
                return -1;
            }
            pm->map[z] = f - trans;
            ++z;
    ...
    for (y = 0; y < pm->h; ++y)
        for (x = 0; x < pm->w; ++x)
        {
            pm->hit[y * pm->w + x] = pm->hitinfo[pm->map[y * pm->w + x]];
        }
    return 0;
}
