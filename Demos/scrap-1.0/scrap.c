...
PRIVATE scrap_type convert_format(int type)
{
    switch (type)
    {
    case T('T', 'E', 'X', 'T'):
...
PRIVATE int convert_data(int type, char *dst, char *src, int srclen)
{
    int dstlen;
...
PRIVATE int convert_scrap(int type, char *dst, char *src, int srclen)
{
    int dstlen;
...
PUBLIC void put_scrap(int type, int srclen, char *src)
{
    scrap_type format;
    int dstlen;
    char *dst;
    ...
    if (dst != NULL)
    {
        ...
        XChangeProperty(SDL_Display, DefaultRootWindow(SDL_Display), XA_CUT_BUFFER0, format, 8, PropModeReplace, dst,
                        dstlen);
        free(dst);
        if (lost_scrap())
        {
...
PUBLIC void get_scrap(int type, int *dstlen, char **dst)
{
    ...
    *dstlen = 0;
    ...
    /* * */
    {
        ...
        Lock_Display();
        if (XGetWindowProperty(SDL_Display, owner, selection, 0, INT_MAX / 4, False, format, &seln_type, &seln_format,
...
PRIVATE int clipboard_filter(const SDL_Event *event)
{
    ...
    case SelectionRequest:
    {
        ...
        if (XGetWindowProperty(SDL_Display, DefaultRootWindow(SDL_Display), XA_CUT_BUFFER0, 0, INT_MAX / 4, False,
                               req->target, &sevent.xselection.target, &seln_format, &nbytes, &overflow, &seln_data) == Success)
        {
            if (sevent.xselection.target == req->target)
            {
                ...
                XChangeProperty(SDL_Display, req->requestor, req->property, sevent.xselection.target, seln_format,
