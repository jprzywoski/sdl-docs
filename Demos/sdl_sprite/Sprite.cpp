...
Sprite::Sprite(SDL_Surface *surface, int frames, int speed)
{
    ...
    else
    {
        ...
        {
            sprite = SDL_CreateRGBSurfaceFrom(surface->pixels, surface->w, surface->h, surface->format->BitsPerPixel,
                                              surface->pitch, surface->format->Rmask, surface->format->Gmask, surface->format->Bmask, 0);
        }
        else
        {
            sprite = SDL_CreateRGBSurfaceFrom(surface->pixels, surface->w, surface->h, surface->format->BitsPerPixel,
                                              surface->pitch, surface->format->Rmask, surface->format->Gmask, surface->format->Bmask,
...
SDL_Surface *SpriteEffects::getRect(Sprite &sprite, int x, int y, int w, int h)
{
    ...
    if (sprite.getSurface()->flags & SDL_SRCCOLORKEY)
    {
        newrect = SDL_CreateRGBSurface(SDL_SWSURFACE, w, h, sprite.getSurface()->format->BitsPerPixel,
                                       sprite.getSurface()->format->Rmask, sprite.getSurface()->format->Gmask, sprite.getSurface()->format->Bmask,
    ...
    for (int y = inter_y0; y <= inter_y1; y++)
    {
        for (int x = inter_x0; x <= inter_x1; x++)
        {
            ...
            if ((!spriteB.isTransparentPixel(x - aX + spriteB.getFrame() * spriteB.getWidth(), y - bY)) &&
                    (!spriteA.isTransparentPixel(x - aX + spriteA.getFrame() * spriteA.getWidth(),
                                                 y - aY)))    /*before pass to isTransparentPixel*/
            {
                return true;
            }
        }
...
void SpriteEffects::flip(Sprite &sprite, int val)
{
    ...
    if (sprite.getSurface()->flags & SDL_SRCCOLORKEY)
    {
        flipped = SDL_CreateRGBSurface(SDL_SWSURFACE, sprite.getSurface()->w, sprite.getSurface()->h,
                                       sprite.getSurface()->format->BitsPerPixel, sprite.getSurface()->format->Rmask,
                                       sprite.getSurface()->format->Gmask, sprite.getSurface()->format->Bmask, sprite.getSurface()->format->Amask);
    }
    else
    {
        flipped = SDL_CreateRGBSurface(SDL_SWSURFACE, sprite.getSurface()->w, sprite.getSurface()->h,
                                       sprite.getSurface()->format->BitsPerPixel, sprite.getSurface()->format->Rmask,
                                       sprite.getSurface()->format->Gmask, sprite.getSurface()->format->Bmask, 0);
    }
...
void SpriteEffects::rotate(Sprite &sprite, int deg)
{
    ...
    for (int i = 0; i < sprite.getMaxFrames(); i++)
    {
        for (int y = 0; y < sprite.getHeight(); y++)
        {
            ...
            {
                if (deg == 90)
                    setPixel(rotated, sprite.getHeight() * i + sprite.getHeight() - y - 1, x, sprite.getPixel((i *
                             sprite.getWidth()) + x, y));
                else if (deg == 180)
                    setPixel(rotated, sprite.getWidth() * i + sprite.getWidth() - x - 1, rotated->h - y - 1,
                             sprite.getPixel((i * sprite.getWidth()) + x, y));
                else if (deg == 270)
                    setPixel(rotated, sprite.getHeight() * i + y, rotated->h - x - 1, sprite.getPixel((i *
                             sprite.getWidth()) + x, y));
                else
                {
                    return;
...
void SpriteEffects::reverseAnimation(Sprite &sprite)
{
    ...
    if (sprite.getSurface()->flags & SDL_SRCCOLORKEY)
    {
        reversed = SDL_CreateRGBSurface(SDL_SWSURFACE, sprite.getSurface()->w, sprite.getSurface()->h,
                                        sprite.getSurface()->format->BitsPerPixel, sprite.getSurface()->format->Rmask,
                                        sprite.getSurface()->format->Gmask, sprite.getSurface()->format->Bmask, sprite.getSurface()->format->Amask);
    }
    else
    ...
    for (int f = 0; f < sprite.getMaxFrames(); f++)
    {
        ...
        {
            for (int x = 0; x < sprite.getWidth(); x++)
                setPixel(reversed, (sprite.getMaxFrames() - f - 1) * sprite.getWidth() + x, y, sprite.getPixel(f *
                         sprite.getWidth() + x, y));
        }
    }
    // Copy color key
...
void SpriteEffects::stretch(Sprite &sprite, float stretchX, float stretchY)
{
    ...
    if (sprite.getSurface()->flags & SDL_SRCCOLORKEY)
    {
        zoomed = SDL_CreateRGBSurface(SDL_SWSURFACE, sprite.getSurface()->w * stretchX, sprite.getSurface()->h *
                                      stretchY, sprite.getSurface()->format->BitsPerPixel, sprite.getSurface()->format->Rmask,
                                      sprite.getSurface()->format->Gmask, sprite.getSurface()->format->Bmask, sprite.getSurface()->format->Amask);
    }
    else
    {
    ...
    for (int y = 0; y < zoomed->h; y++)
    {
        for (int x = 0; x < zoomedWidth; x++)
        {
            ...
            for (int i = 0; i < sprite.getMaxFrames(); i++)
                setPixel(zoomed, (zoomedWidth * i) + x, y, sprite.getPixel((sprite.getWidth() * i) + (int)(x /
                         stretchX), (int)(y / stretchY)));
        }
    }
