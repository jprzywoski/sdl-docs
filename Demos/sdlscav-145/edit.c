...
void mode4()                            /* level editor */
{
    ...
    else if (checkdown(SDLK_LEFT))      /* left arrow */
    {
        numkey = 0;
        if (clevel > 1)
        {
    ...
    else if (checkdown(SDLK_RIGHT))     /* right arrow */
    {
        numkey = 0;
        if (clevel < (MAXLEVELS))
        {
    ...
    else if (checkdown(SDLK_UP))        /* up arrow */
    {
        numkey = 0;
    ...
    else if (checkdown(SDLK_DOWN))      /* down arrow */
    {
        numkey = 0;
        if (clevel > 10)
        {
    ...
    for (i = 0; i <= 9; i++)
        if ((checkdown(numpadmap[i]) || checkdown(numbermap[i])) && !emodified)         /* #0-9 */
        {
            if ((i == 0) && (numkey == 0))
            {
                break;    /* leading zeros do nothing */
            }
            if (numkey == 0)
            {
                numkey = 1;
    ...
    else if (checkdown(SDLK_KP_MINUS) || checkdown(SDLK_MINUS))
    {
        if (emodified)
        {
            centermsg("Save or load before changing levels");
    ...
    else if (checkdown(SDLK_l) || checkdown(SDLK_KP_ENTER) || checkdown(SDLK_RETURN))           /* l */
    {
        numkey = 0;
        if (clevel == 0)
        {
    ...
    if (checkdown(SDLK_w))              /* w */
    {
        ...
        else
        {
            ...
            if (i && !(checkpressed(SDLK_LALT) || checkpressed(SDLK_RALT)))
            {
...
void editprocess()
{
    ...
    if ((checkpressed(SDLK_LALT) || checkpressed(SDLK_RALT)) && checkdown(SDLK_f))      /* alt-f */
    {
        numkey = 0;
