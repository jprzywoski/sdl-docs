...
int soundopen(void)
{
    ...
    char *soundnames[NUM_SOUNDS] = { "fall.wav", "dig.wav", "pop.wav", "victory.wav", "death.wav" };
    char file[256];
    Uint8 *data;
    Uint32 dlen;
    SDL_AudioSpec fmt;
    ...
    for (num = 0; num < NUM_SOUNDS; num++)
    {
        ...
        for (i = 0;; i++)                                       /* Check all paths */
        {
            ...
            if (0 == stat(file, &Exists))
            {
                break;    /* If file exists exit loop */
            }
        }
        /* Use standard SDL library to load standard format wave file. */
        if (SDL_LoadWAV(file, &wave, &data, &dlen) == NULL)     /* If the format was bad exit with an error message */
        {
        ...
        SDL_BuildAudioCVT(&cvt[num], wave.format, wave.channels, wave.freq, AUDIO_U8, 1, 11025);
...
void playsound(int num)
{
    if (!soundworking)
    {
        return;    /* Exit early if sound disabled */
    }
    if ((num < 0) || (num >= NUM_SOUNDS))
    {
        return;    /* Exit early invalid sound num */
    }
    /* If the sound is still busy kill it */
    if (sounds[num].dpos != sounds[num].dlen)
    {
        StopSound(num);
    }
